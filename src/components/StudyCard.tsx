...
export const StudyCard: React.FC<StudyCardProps> = ({
  ...
  // Color mapping based on 5 Math Domains
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-4 flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between mb-4">
        <button
          onClick={onExit}
          className="min-h-11 min-w-11 px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 text-xs font-semibold flex items-center gap-1 transition"
        >
      ...
      >
        <div
          className={`w-full h-full min-h-[380px] sm:min-h-[420px] rounded-3xl p-6 sm:p-8 border shadow-lg transition-transform duration-500 preserve-3d flex flex-col justify-between ${
            isFlipped ? 'rotate-y-180 bg-slate-900 text-white border-slate-800' : 'bg-white text-slate-900 border-slate-200'
          }`}
        >
          {/* Card Front */}
          {!isFlipped ? (
            <div className="flex flex-col justify-between h-full">
              ...
              <div className="my-auto py-6 text-center">
                <div className="text-xs uppercase font-bold text-blue-600 tracking-wider mb-2">
                  QUESTION / 개념 질문
                </div>
                <KaTeXView
                  content={frontText}
                  className="text-lg sm:text-2xl font-semibold text-slate-800"
                />
              </div>
      ...
      <div className="w-full mt-6 pb-safe">
        {!isFlipped ? (
          <button
            onClick={handleFlip}
            className="w-full min-h-12 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl shadow-md text-base transition flex items-center justify-center gap-2 active:scale-[0.98]"
          >
          ...
          <div className="grid grid-cols-4 gap-2 sm:gap-3">
            <button
              ...
              className="min-h-12 py-2 px-1 bg-red-50 hover:bg-red-100 active:scale-95 border border-red-200 text-red-700 font-bold rounded-xl text-xs sm:text-sm flex flex-col items-center justify-center transition"
            >
              <span>{t('ratingAgain')}</span>
            </button>
            <button
              onClick={() => handleRate(2)}
              className="min-h-12 py-2 px-1 bg-amber-50 hover:bg-amber-100 active:scale-95 border border-amber-200 text-amber-700 font-bold rounded-xl text-xs sm:text-sm flex flex-col items-center justify-center transition"
            >
              <span>{t('ratingHard')}</span>
            </button>
            <button
              onClick={() => handleRate(3)}
              className="min-h-12 py-2 px-1 bg-blue-50 hover:bg-blue-100 active:scale-95 border border-blue-200 text-blue-700 font-bold rounded-xl text-xs sm:text-sm flex flex-col items-center justify-center transition"
            >
              <span>{t('ratingGood')}</span>
            </button>
            <button
              onClick={() => handleRate(4)}
              className="min-h-12 py-2 px-1 bg-emerald-50 hover:bg-emerald-100 active:scale-95 border border-emerald-200 text-emerald-700 font-bold rounded-xl text-xs sm:text-sm flex flex-col items-center justify-center transition"
            >
